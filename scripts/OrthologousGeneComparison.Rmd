...
data <- set_data_all %>% 
  select(-Hypomyces_Cladobotryum, -Trichoderma, -Escovopsis) %>%
  group_by_all() %>% 
  summarise(count = n()) %>%
  ungroup() %>%
...
plot_1 <- data %>%
  ggplot(aes(x = rows, y = group_name)) +
  geom_hline(yintercept = 'AllOutgroup', size = 10, colour = 'gray') +
  geom_hline(yintercept = 'Lower', size = 10, colour = colors[['Lower']]) +
  geom_hline(yintercept = 'Coral', size = 10, colour = colors[['Coral']]) +
  geom_hline(yintercept = 'Higher', size = 10, colour = colors[['Higher']]) +
  geom_hline(yintercept = 'Leafcutter', size = 10, colour = colors[['Leafcutter']]) +
  geom_point(size = 4) + 
  theme_classic() +
  geom_path(aes(group = rows), size = 1) +
  xlab('') +
  ylab('') +
...
plot_grid(plot_2, plot_1, label_size = 12, ncol = 1, axis = 'r', rel_heights = c(4,1)) +
  ggsave2(filename = 'plots/custom_upset.pdf')
